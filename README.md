...
회의 (2026-05) 기준 — 프론트 관점:

1. **매매일지 화면** — 거래 내역 리스트 + 메모 작성/조회
2. **자산 추이 차트** — 일/주/월/년 단위 자산 변화 (토스·뱅크샐러드 스타일)
3. **고래 추적 페이지** — 지갑 이벤트 타임라인
4. **거래소 추가 UI** — 국내 거래소 + 코인베이스 OAuth 연동 폼
5. **배포** — 백엔드 GCP 배포에 맞춰 프론트도 배포
6. **AI 코멘트** (이후) — 매매일지 AI 분석 표시

서버 측 로드맵 (동시성 점검·모니터링·부하 테스트 등) 은 [coinavi-server README](https://github.com/intern-poc/coinavi-server#-향후-로드맵) 참조.
