...
| **통합 포트폴리오** | 비중 도넛 + 종목별 평가금액·비중 + 거래소별 필터 탭 (Upbit/Bithumb/전체) |
| **자산 추이 차트** | 일별 스냅샷 시계열 (1주/1개월/1년) + 가입 즉시 과거 1년 백필 + **vs BTC 단순보유 벤치마크** + 원금 표시 |
| **매매 성과** | 실현손익·승률·평균 보유기간 + 코인별 실현손익 막대 + 베스트/워스트 거래 |
| **거래소 키 관리** | AES-256-GCM 백엔드 암호화 · Upbit/Bithumb 가이드 탭 · 시연용 Demo (dev) · 평문 절대 노출 X |
...
| `/portfolio` | 필요 | 통합 자산 (비중·자산추이·매매성과 토글 + 종목 비중 + 거래소 필터) |
...
│   ├── exchange-keys/            # API 키 CRUD + 거래소 가이드 (Upbit/Bithumb/Demo)
│   └── portfolio/                # 비중·자산추이·매매성과 토글 + 종목 리스트 + 거래소 필터 + polling
├── lib/
...
- **자산 추이 차트** — 일별 스냅샷 + 과거 1년 백필 + 1주/1개월/1년 토글 + vs BTC 벤치마크 + 원금 표시
- **매매 성과** — 실현손익·승률·보유기간 + 코인별 막대 (비중·자산추이와 한 카드 토글)
- **거래소 필터** — Upbit/Bithumb/전체 탭 + 거래소 키 가이드(Bithumb·Demo 추가)

### 🔜 예정

1. **매매일지 화면** — 청산 거래 리스트 + 메모 작성/조회 (매매 성과의 거래 단위 확장)
2. **고래 추적 페이지** — 지갑 이벤트 타임라인
3. **거래소 추가 UI** — 코인베이스 OAuth 연동 폼
4. **배포** — 백엔드 GCP 배포에 맞춰 프론트도 배포
5. **AI 코멘트** (이후) — 매매일지 AI 분석 표시

서버 측 로드맵 (동시성 점검·모니터링·부하 테스트 등) 은 [coinavi-server README](https://github.com/intern-poc/coinavi-server#%EF%B8%8F-로드맵) 참조.
